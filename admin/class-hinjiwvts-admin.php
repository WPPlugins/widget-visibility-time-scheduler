...
class Hinjiwvts_Admin {
	...
	public function enqueue_styles( $hook_suffix ) {
		...
		if ( 'widgets.php' == $hook_suffix ) {
			if( is_rtl() ) {
				wp_enqueue_style( $this->plugin_slug, plugin_dir_url( __FILE__ ) . 'css/hinjiwvts-admin-rtl.css', array(), $this->plugin_version, 'all' );
			} else {
				wp_enqueue_style( $this->plugin_slug, plugin_dir_url( __FILE__ ) . 'css/hinjiwvts-admin.css', array(), $this->plugin_version, 'all' );
			}
		}
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		if ( 'widgets.php' == $hook_suffix ) {
			...
			wp_enqueue_script( $this->plugin_slug, plugin_dir_url( __FILE__ ) . 'js/hinjiwvts-admin.js', array( 'jquery' ), $this->plugin_version, false );
	...
	private function touch_time( $boundary ) {
		...
		for ( $i = 1; $i < 13; $i = $i +1 ) {
			...
			$month .= sprintf( esc_html__( $label ), $monthnum, $wp_locale->get_month_abbrev( $wp_locale->get_month( $i ) ) ) . '</option>';
		}
		$month .= '</select>';
